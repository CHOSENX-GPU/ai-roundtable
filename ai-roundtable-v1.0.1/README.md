...
**重大更新 - Web App 架构**:
- 🌐 新增独立 Web App 操作界面（React + Vite + Tailwind）
- 🔗 Extension Bridge：Web App 与扩展通过安全配对机制通信
- 📱 全新 Light Compact Minimalist UI 设计（Vercel/Linear 风格）
- 🎯 侧边栏精简为配对面板，显示配对码和 AI 连接状态

**技术改进**:
- 🔐 配对机制：6 位配对码 + Token 认证
- 📡 实时状态同步：AI 连接状态自动推送到 Web App
- 🏗️ 模块化架构：hooks、components、lib 分离

...
├── background.js           # Service Worker + External Bridge
├── sidepanel/
│   ├── panel.html         # 配对面板 UI
│   ├── panel.css          # 配对面板样式
│   └── panel.js           # 配对逻辑
...
**重大 UI 重构 - Light Compact 设计系统**:
- 🎨 全面采用 Light Compact 设计系统（类 Vercel/Linear 风格）
- ✨ AI 卡片增强：添加品牌 Logo 图标，连接状态可视化（绿点/红点）
- 🎯 Mention 按钮品牌色：永久显示 AI 品牌色，悬停时发光效果
- 🔄 智能点击交互：未连接时点击卡片打开 AI 网站，已连接则无操作
- 📐 布局优化：压缩间距，文本框扩展至 150px，按钮右对齐
...
**交互改进**:
- 模式切换按钮：普通/讨论模式激活时显示深灰色背景 (#475569)
- AI 卡片连接状态：显示品牌色（连接时）+ 加粗显示（选中时）
- 文本框与活动日志距离优化：gap 从 12px 缩减至 6px
- 按钮布局：新对话和发送按钮独立位于文本框下方右对齐

**技术细节**:
- 清理重复 CSS 规则
- 优化悬停效果：品牌色按钮使用 `box-shadow` 和 `currentColor` 实现发光
- SVG 图标集成：从 "../AI icon/" 文件夹加载
- 连接点视觉设计：8px 圆点，绿色 (#10B981) 表示连接，红色 (#EF4444) 表示未连接

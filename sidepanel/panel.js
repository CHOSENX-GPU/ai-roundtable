...
function updateTabStatus(aiType, connected) {
  // Update normal mode connection dot
  const connectionDot = document.getElementById(`status-${aiType}`);
  const checkbox = document.getElementById(`target-${aiType}`);
  const card = checkbox ? checkbox.parentElement : null;
  ...
  // Update discussion mode connection dot
  const discConnectionDot = document.getElementById(`disc-status-${aiType}`);
  const discCheckbox = document.getElementById(`disc-participant-${aiType}`);
  const discCard = discCheckbox ? discCheckbox.parentElement : null;

  if (discConnectionDot) {
    if (connected) {
      discConnectionDot.classList.add('connected');
    } else {
      discConnectionDot.classList.remove('connected');
    }
  }
  ...
  if (connected) {
    ...
    // Enable and show mention button
    if (mentionBtn) {
    ...
    if (discCheckbox) {
      ...
      if (discCard) discCard.classList.remove('connected');
    }
    // Disable and hide mention button
    if (mentionBtn) {
...
function setupHelpDropdown() {
  // Help dropdown is already set up in setupEventListeners
  // This function is reserved for future help-related functionality
}

function setupAICardClickHandlers() {
  // Add click handlers to AI cards for opening chat websites
  document.querySelectorAll('.target-label.compact[data-url]').forEach(card => {
    card.addEventListener('click', (e) => {
      // Only open URL if not clicking on checkbox or span
      if (e.target.tagName !== 'INPUT' && e.target.tagName !== 'SPAN') {
        const url = card.dataset.url;
        const aiName = card.dataset.ai;

        // Only open if NOT connected (if connected, user already has the AI open)
        if (!connectedTabs[aiName]) {
          chrome.tabs.create({ url: url });
          log(`已打开 ${aiName} 网站`, 'success');
        }
      }
    });
  });
